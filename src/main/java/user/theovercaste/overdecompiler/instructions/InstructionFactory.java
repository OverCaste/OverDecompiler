...
public enum InstructionFactory {
    ...
    private final ImmutableMap<Integer, Instruction.Factory> factoryMap = getFactoryMap();

    private ImmutableMap<Integer, Instruction.Factory> getFactoryMap( ) {
    ...
    private void registerInstruction(ImmutableMap.Builder<Integer, Instruction.Factory> builder, int[] opcodes, Instruction.Factory b) {
        for (int i : opcodes) {
            builder.put(i, b); // Duplicate keys already fail
        }
    }

    public Instruction.Factory getFactory(int id) throws IOException {
